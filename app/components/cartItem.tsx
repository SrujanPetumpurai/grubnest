...
export default function CartItem({id,quantity,name,image,category,cost,discount}:{quantity:number,id:string,name:string,image:string,cost:number,category:string,discount:string|null}){
    ...
    return(
        <div className="flex w-[450px] border-b pb-4 mt-2">
            <div className="w-[20%] h-full py-2 px-2">
                <img src={image} alt="" />
            </div>
            <div className="flex flex-col w-[85%] pr-2 py-2">
                <div className="flex justify-between">
                    <span className="inline-block text-lg font-bold ">{name}</span>
                    <span className="inline-block w-[20px] h-[20px]">
                        <svg onClick={()=>deleteItem()} className="w-full h-full" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" fill="#000000">
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                            <title>delete_2_line</title>
                            <g id="页面-1" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                            <g id="System" transform="translate(-576.000000, -192.000000)" fillRule="nonzero">
                ...
                <span className="inline-block text-sm font-light">{category}</span>
                <div className="flex justify-between">
                    <div>
                        <span className="inline-block text-lg font-bold">{cost}Rs</span>
                        <span className="ml-2 text-red-300">{discount}</span>
                    </div>
                    <div><AddCart cl='black' bg="white" id={id} Inquantity={quantity}></AddCart></div>
                </div>
            </div>
        </div>
    )
